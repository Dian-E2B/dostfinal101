...
class DashboardController extends Controller
{
    ...
    public function dashboardview()
    {
        ...
        $ongoingRecords = DB::table('ongoing')
            ->where('scholarshipprogram', 'merit')
            ->groupBy('schools')
            ->get();

        // Now $ongoingRecords contains the result set grouped by schools


        $allowedSchools = ['ADDU', 'Brokenshire', 'DDC', 'SPC', 'UIC', 'UM-Matina', 'UP Mindanao', 'USEP Mintal', 'USEP-Obrero'];

        $ongoingRecords1 = DB::table('ongoing')
            ->where('scholarshipprogram', 'merit')
            ->whereIn('schools', $allowedSchools)
            ->groupBy('schools')
            ->get()
            ->toJson();
